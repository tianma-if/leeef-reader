...
- `ReaderScreen`：恢复阅读位置、自动保存、目录、书签、文本选择书摘，以及移动端 Page Curl/普通翻页降级。
...
flutter test integration_test/m1_vertical_slice_test.dart -d <iPhone device>
  iPhone 真机阅读、Page Curl、相邻页纹理与数据同步闭环通过
...
flutter test integration_test/m1_vertical_slice_test.dart -d emulator-5554
  Android 16 ARM 阅读、Page Curl、相邻页纹理与数据同步闭环通过

GitHub Actions Leeef Windows verification #32551238649
  Windows WebView2 阅读、32 项 Flutter 测试、Go/MCP 和 M1 数据纵向测试通过
```

Windows CI：[Leeef Windows verification #32551238649](https://github.com/tianma-if/leeef-reader/actions/runs/32551238649)
