...
| 移动端 Page Curl 与降级 | 通过 | iOS/Android 的 EPUB 使用独立 foliate WebView、TXT 使用 Canvas 预渲染当前与相邻页纹理并交给 Fragment Shader；纹理或 Shader 不可用时自动执行普通翻页 |
...
flutter test integration_test/required_formats_test.dart -d macos
  EPUB 阅读 3 项 + M1 数据纵向闭环 1 项 + PDF 阅读恢复、TXT Page Curl 与恢复 2 项通过
...
flutter test integration_test/required_formats_test.dart -d <iPhone device>
  iPhone 真机 EPUB/TXT Page Curl、相邻页纹理、PDF/TXT 阅读恢复与数据同步闭环通过
