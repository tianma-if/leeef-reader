...
void main() {
  ...
  if (projected_normal >= 0.0 && projected_normal <= u_radius) {
    ...
    float max_tail = max(-dot(u_touch - u_fold_center, u_normal), 0.0);
    has_back = -projected_normal <= max_tail + 0.5;
  }
  ...
  vec2 base_uv = sample_uv(screen / u_size);
  vec4 color;
  if (has_back) {
    vec4 ink = texture(u_current_page, texture_uv(back_source));
    float grazing = abs(sin(angle));
    float paper_light = 0.86 + grazing * 0.10;
    vec3 warm_paper = vec3(1.0, 0.965, 0.875);
    color = vec4(mix(ink.rgb * paper_light, warm_paper, 0.12), ink.a);
    ...
    color.rgb = mix(color.rgb, vec3(1.0, 0.99, 0.94), edge_highlight * 0.55);
