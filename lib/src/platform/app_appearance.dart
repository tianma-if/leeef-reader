...
class AppStrings {
  ...
  static const _englishText = <String, String>{
    ...
    '选择阿里云、腾讯云、七牛或任意 S3 兼容服务':
        'Choose Aliyun, Tencent Cloud, Qiniu, or any S3-compatible service',
    '按云厂商填写控制台里的同名项目，不用自己拼 Endpoint。':
        'Fill in the same fields as your cloud console. You do not need to write the endpoint yourself.',
    '阿里云 OSS': 'Aliyun OSS',
    ...
    '存储路径': 'Storage path',
    '高级选项': 'Advanced',
    '自定义 Endpoint、Path-style、临时密钥':
        'Custom endpoint, path-style, and session token',
    '自定义 Endpoint': 'Custom endpoint',
    '检测连接': 'Test connection',
    '检测对象存储': 'Test object storage',
    '请填写完整的存储配置。': 'Please complete the storage settings.',
    '书籍会放在这个目录下，一般不用改':
        'Books are stored in this path. You can usually leave the default.',
    '与控制台中的密钥名称一致': 'Use the same key names as in the console',
    '控制台里 Bucket 的名称，不要填网址': 'The bucket name from the console, not a URL',
    '与 Bucket 概览里的地域一致': 'Match the region shown in the bucket overview',
    'R2 控制台概览页中的 Account ID': 'The Account ID from the R2 overview page',
    'MinIO 或 NAS 上的 S3 地址': 'The S3 endpoint on MinIO or your NAS',
    '请先配置对象存储。': 'Configure object storage first.',
    '请先在设置中配置对象存储。': 'Configure object storage in Settings first.',
    '打开阿里云 OSS 控制台，把 Bucket 名称、地域和 AccessKey 填进来。不用手写 Endpoint。':
        'Open the Aliyun OSS console and fill in the bucket, region, and AccessKey. No endpoint required.',
    '存储桶名通常是 bucket-appid，例如 example-1250000000。地域与 COS 控制台一致。':
        'The bucket name is usually bucket-appid, such as example-1250000000. Use the COS console region.',
    '使用七牛的 S3 兼容访问。空间名、密钥和区域都可以在 Kodo 控制台直接复制。':
        'Uses Qiniu S3-compatible access. Copy the bucket, keys, and region from the Kodo console.',
    '地域选择与 OBS 桶概览里的区域一致，例如华北-北京四。':
        'Choose the same region as the OBS bucket overview, such as cn-north-4.',
    '使用 TOS 的 S3 兼容 Endpoint。地域填控制台中的 Region，例如 cn-beijing。':
        'Uses the TOS S3-compatible endpoint. Fill in the console region, such as cn-beijing.',
    '选择 Bucket 所在区域即可，Endpoint 会按区域自动生成。':
        'Pick the bucket region; the endpoint is generated automatically.',
    'Account ID 在 R2 概览页，用来生成 Endpoint。密钥在 R2 的 S3 API 令牌里创建。':
        'The Account ID is on the R2 overview page and builds the endpoint. Create keys with an R2 S3 API token.',
    '适用于 MinIO、群晖、威联通等兼容服务。填写局域网或公网的 S3 地址。':
        'For MinIO, Synology, QNAP, and similar services. Enter the LAN or public S3 URL.',
    '其他 S3 兼容服务可在这里填写完整 Endpoint 和 Region。':
        'Use this for other S3-compatible services and fill in the full endpoint and region.',
    '华东1（杭州）': 'China East 1 (Hangzhou)',
    '华东2（上海）': 'China East 2 (Shanghai)',
    '华东5（南京）': 'China East 5 (Nanjing)',
    '华北1（青岛）': 'China North 1 (Qingdao)',
    '华北2（北京）': 'China North 2 (Beijing)',
    '华南1（深圳）': 'China South 1 (Shenzhen)',
    '华南3（广州）': 'China South 3 (Guangzhou)',
    '西南1（成都）': 'China Southwest 1 (Chengdu)',
    '中国香港': 'Hong Kong',
  ...
  static const _japaneseText = <String, String>{
    ...
    '选择阿里云、腾讯云、七牛或任意 S3 兼容服务': 'Aliyun、Tencent Cloud、Qiniu、その他の S3 互換サービスを選択',
    '按云厂商填写控制台里的同名项目，不用自己拼 Endpoint。':
        'クラウドコンソールと同じ項目を入力します。Endpoint を自分で書く必要はありません。',
    '阿里云 OSS': 'Aliyun OSS',
    '腾讯云 COS': 'Tencent Cloud COS',
    '七牛云 Kodo': 'Qiniu Kodo',
    '华为云 OBS': 'Huawei Cloud OBS',
    ...
    '存储路径': '保存パス',
    '高级选项': '詳細設定',
    '自定义 Endpoint、Path-style、临时密钥': 'カスタム Endpoint、Path-style、セッショントークン',
    '自定义 Endpoint': 'カスタム Endpoint',
    '检测连接': '接続をテスト',
    '检测对象存储': 'オブジェクトストレージをテスト',
    '请填写完整的存储配置。': 'ストレージ設定をすべて入力してください。',
    '书籍会放在这个目录下，一般不用改': '本はこのパスに保存されます。通常は変更不要です。',
    '与控制台中的密钥名称一致': 'コンソールのキー名と同じものを入力',
    '控制台里 Bucket 的名称，不要填网址': 'コンソールのバケット名。URL は入力しないでください',
    '与 Bucket 概览里的地域一致': 'バケット概要のリージョンと一致させてください',
    'R2 控制台概览页中的 Account ID': 'R2 概要ページの Account ID',
    'MinIO 或 NAS 上的 S3 地址': 'MinIO または NAS の S3 アドレス',
    '请先配置对象存储。': '先にオブジェクトストレージを設定してください。',
    '请先在设置中配置对象存储。': '設定でオブジェクトストレージを先に構成してください。',
    '打开阿里云 OSS 控制台，把 Bucket 名称、地域和 AccessKey 填进来。不用手写 Endpoint。':
        'Aliyun OSS コンソールでバケット、リージョン、AccessKey を入力します。Endpoint は不要です。',
    '存储桶名通常是 bucket-appid，例如 example-1250000000。地域与 COS 控制台一致。':
        'バケット名は通常 bucket-appid 形式です。COS コンソールのリージョンを使います。',
    '使用七牛的 S3 兼容访问。空间名、密钥和区域都可以在 Kodo 控制台直接复制。':
        'Qiniu の S3 互換アクセスを使います。バケット、キー、リージョンは Kodo コンソールからコピーできます。',
    '地域选择与 OBS 桶概览里的区域一致，例如华北-北京四。': 'OBS バケット概要と同じリージョンを選んでください。',
    '使用 TOS 的 S3 兼容 Endpoint。地域填控制台中的 Region，例如 cn-beijing。':
        'TOS の S3 互換 Endpoint を使います。コンソールのリージョンを入力します。',
    '选择 Bucket 所在区域即可，Endpoint 会按区域自动生成。': 'バケットのリージョンを選べば Endpoint は自動生成されます。',
    'Account ID 在 R2 概览页，用来生成 Endpoint。密钥在 R2 的 S3 API 令牌里创建。':
        'Account ID は R2 概要ページにあり、Endpoint の生成に使います。キーは R2 の S3 API トークンで作成します。',
    '适用于 MinIO、群晖、威联通等兼容服务。填写局域网或公网的 S3 地址。':
        'MinIO、Synology、QNAP などの互換サービス向けです。LAN または公開 S3 URL を入力します。',
    '其他 S3 兼容服务可在这里填写完整 Endpoint 和 Region。':
        'その他の S3 互換サービスは、完全な Endpoint とリージョンを入力します。',
    '华东1（杭州）': '中国東部1（杭州）',
    '华东2（上海）': '中国東部2（上海）',
    '华北2（北京）': '中国北部2（北京）',
    '华南1（深圳）': '中国南部1（深圳）',
