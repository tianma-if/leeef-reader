...
9. **商店交付**：按用户指定的目标渠道交付；用户要求正式上架时，可直接发布 Google Play production 或提交 App Store 审核，不要求先经过 internal、draft、TestFlight 测试分发或真机回归。平台自身要求的构建处理、资料与审核仍需完成。商店上传或送审不等于正式上架，商店状态必须单独核验。
