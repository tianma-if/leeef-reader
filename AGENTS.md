...
Google Play production 与 iOS App Store Connect 上传已接到 `.github/workflows/tauri-release.yml`。不要用旧的 `google-play.yml` / `ios-app-store.yml` / `macos-dmg.yml` 对正式 Tag 发版。Sparkle `appcast.xml` 仍未从 Flutter 迁完。
