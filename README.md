...
设置中可生成配对二维码。同一局域网内的手机扫描后，即可同步这台电脑已配置的存储、AI、TTS、OPDS 和阅读设置。也可在“我的同步设备”中使用一次性配对码。已信任设备之间使用独立设备密钥和 AES-256-GCM 端到端加密配置；任意已有设备均可帮助新设备恢复配置。撤销设备时会轮换同步空间密钥，旧设备无法解密后续配置。
