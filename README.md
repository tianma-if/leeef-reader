...
| 可迁移配置 | 阅读外观、AI/TTS/OPDS、S3/WebDAV 设置及加密凭据 |

应用离线时正常读写；恢复网络后自动同步。书籍文件按 SHA-256 去重，元数据通过增量操作日志同步，不直接上传或覆盖 SQLite 数据库文件。

对象存储、AI、TTS 和类似服务凭据只在桌面端填写。设置中可生成配对二维码；同一局域网内的手机扫描后，即可同步这台电脑已配置的存储、AI、TTS、OPDS 和阅读设置。手机端不提供这些凭据的填写界面。也可在“我的同步设备”中使用一次性配对码。已信任设备之间使用独立设备密钥和 AES-256-GCM 端到端加密配置；任意已有设备均可帮助新设备恢复配置。撤销设备时会轮换同步空间密钥，旧设备无法解密后续配置。
...
trusted/{spaceId}/devices          加密设备清单
trusted/{spaceId}/config           各设备加密配置快照
trusted/{spaceId}/keys             设备撤销与密钥轮换
```

S3 是默认后端；WebDAV 作为可选适配器。启动同步前执行能力检测，确认目录创建、上传、下载、条件写入和删除行为。同步后端保存不可变文件、操作日志、水位及端到端加密的设备与配置文档，不上传或覆盖 SQLite 数据库文件。
...
- [x] foliate-js 四端加载、排版、CFI 和文本选择
