...
│
├── 翻译
│   ├── 划词翻译：携带上下文、自动翻译、源/目标语言切换
│   ├── 全文翻译：原文、译文、双语对照
│   └── 服务：Bing Web、Google Web、Microsoft、Google Cloud、DeepL、AI
│
├── TTS 朗读
...
### Phase 1：阅读内核与移动端 3D 翻页

- [ ] Flutter 四端工程骨架
- [ ] EPUB、PDF、TXT 导入与阅读
- [ ] foliate-js `ReaderEngine` 与安全 loopback 资源服务
- [ ] 统一页面模型
- [ ] iOS / Android WebView 相邻页预渲染与页面快照原型
- [ ] iOS / Android 3D Page Curl 手势、Shader 和双页纹理缓存
- [ ] macOS / Windows 滑动、无动画和连续滚动模式
- [ ] 阅读进度、书摘、书签和书架目录本地存储
